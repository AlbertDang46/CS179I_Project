    ...
    while (!quit){
        ...
        if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
            ...
            if (rte_mempool_get(message_pool, &msg) < 0) {
                ...
                strlcpy((char *) msg, sendThis, STR_TOKEN_SIZE);
...
main(int argc, char **argv)
{
    ...
    const unsigned pool_size = 1024; // size
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY){
        ...
        if (rte_mempool_get(message_pool, &msg) < 0) {
            ...
            strlcpy((char *) msg, message, STR_TOKEN_SIZE);
...
- use rte_malloc to allocate memory in memory pool
